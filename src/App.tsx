...
// --- CONFIGURAÇÃO CORE E BANCO DE DADOS ---
...
export default function CRMMasterSystemUltra() {
  ...
  // --- PARAMETRIZAÇÃO DE REGRAS DE NEGÓCIO (COMISSÃO) ---
  ...
  const [commSettings, setCommSettings] = useState({
    ...
    profitMargin: 15 // Trava de margem: Se 0, zera a comissão.
  });
  ...
  useEffect(() => {
    fetchLeads();
    if (!supabase) return;
    const channel = supabase.channel('crm_full_sync').on('postgres_changes', { event: '*', schema: 'public', table: 'leads' }, fetchLeads).subscribe();
  ...
  const handleSaveLead = async (leadData) => {
    ...
    if (error) alert("Erro de Sincronia: " + error.message);
    setIsSaving(false);
  };

  const deleteLead = async (id) => {
    if (!supabase || !window.confirm("ATENÇÃO: Deseja apagar permanentemente este registro?")) return;
  ...
  // --- MOTOR ANALYTICS AVANÇADO (CÁLCULOS TÉCNICOS) ---
  const analytics = useMemo(() => {
    ...
    // --- REGRAS DE COMISSÃO (ALGORITMO MASTER) ---
    ...
    // 3. Bônus Fixo R$ 300 (Condição All-In)
  ...
  if (loading) return <div className="h-screen bg-slate-900 flex items-center justify-center font-black text-blue-500 animate-pulse uppercase tracking-[1em]">SALESPRO: CARREGANDO ESTRUTURA...</div>;
    ...
    <div className="min-h-screen bg-[#F1F5F9] p-4 md:p-10 font-sans">
      ...
      <header className="max-w-[1800px] mx-auto mb-10 flex flex-col xl:flex-row justify-between items-center gap-10">
        <div className="flex items-center gap-8">
          <div className="bg-slate-900 p-7 rounded-[2.5rem] shadow-2xl group hover:rotate-6 transition-all duration-500">
            <TrendingUp className="text-blue-500" size={40} />
          </div>
          <div>
            <h1 className="text-6xl font-black tracking-tighter text-slate-900 italic leading-none">SALES<span className="text-blue-600">PRO</span> <span className="text-[14px] not-italic text-slate-400 ml-4 font-bold border-l pl-4 border-slate-300">ENTERPRISE v5.0</span></h1>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.6em] mt-2 flex items-center gap-2"><span className="w-2 h-2 bg-emerald-500 rounded-full animate-ping"></span> Real-Time Analytics Connected</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-4 bg-white p-4 rounded-[4rem] shadow-xl border-2 border-white">
          <div className="flex bg-slate-100 p-2 rounded-[2.5rem] mr-4">
            {[1, 2, 3, 4].map(w => (
              <button key={w} onClick={() => setCurrentWeek(w)} className={`px-8 py-3 rounded-2xl font-black text-[11px] transition-all ${currentWeek === w ? 'bg-white text-blue-600 shadow-md scale-110' : 'text-slate-400'}`}>W{w}</button>
            ))}
          </div>
          ...
          <button onClick={() => setIsModalOpen(true)} className="bg-blue-600 text-white p-5 rounded-full shadow-lg hover:rotate-180 transition-all duration-700 hover:scale-110"><PlusCircle size={28} /></button>
        </div>
      </header>

      <main className="max-w-[1800px] mx-auto">
        
        {/* ABA: PIPELINE MASTER (DRAG AND DROP INTEGRAL) */}
        {activeTab === 'pipeline' && (
                    ...
                    {stageLeads.map(lead => {
                      ...
                      return (
                        <div key={lead.id} 
                          ...
                          {expired && <div className="absolute -left-3 top-12 bg-rose-500 text-white p-3 rounded-full shadow-xl animate-bounce z-20"><Clock size={16}/></div>}
                          ...
                          <div className="bg-slate-50 p-6 rounded-[2rem] mb-6 border border-slate-100">
                             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2"><FileText size={12}/> Briefing Técnico</p>
                             <textarea 
           ...
           <div className="bg-white rounded-[4rem] shadow-2xl border-4 border-white overflow-hidden animate-in slide-in-from-bottom-10">
              <div className="bg-slate-900 p-12 text-white flex justify-between items-center">
                 <div className="flex items-center gap-6">
                    ...
                    <div>
                       <h3 className="text-3xl font-black uppercase tracking-tighter">Performance Operacional</h3>
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Monitoramento semanal de esforço</p>
                    </div>
                 </div>
              </div>
        ...
        {/* ABA: COMISSÃO & AUDITORIA INTEGRAL */}
          ...
          <div className="space-y-12 animate-in slide-in-from-bottom-10 pb-20">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <div className="bg-slate-900 p-20 rounded-[5rem] text-white shadow-2xl relative overflow-hidden group">
                 <DollarSign className="absolute -right-20 -top-20 opacity-5" size={500}/>
                 ...
                 <div className="flex gap-16 border-t border-white/10 pt-12">
                    <div><p className="text-[10px] font-black opacity-40 uppercase mb-2">Atingimento Meta</p><p className="text-5xl font-black">{analytics.revPerf.toFixed(1)}%</p></div>
                    <div><p className="text-[10px] font-black opacity-40 uppercase mb-2">Ticket Médio</p><p className="text-5xl font-black">R$ {analytics.avgTicket.toLocaleString()}</p></div>
                 </div>
              </div>

              <div className="bg-white p-20 rounded-[5rem] border-[16px] border-emerald-500 shadow-2xl text-center flex flex-col justify-center items-center">
                 ...
                 <div className="mt-12 flex flex-col items-center gap-6">
                    <div className="bg-emerald-600 text-white px-12 py-4 rounded-full font-black text-sm uppercase tracking-widest shadow-2xl flex items-center gap-4">Taxa de Performance: {analytics.finalRate.toFixed(2)}%</div>
                    {analytics.bonusHabilitado && <div className="text-emerald-500 font-black text-xs uppercase animate-pulse border-b-2 border-emerald-500 pb-1">+ Bônus Operacional (R$ 300,00) Conquistado ✓</div>}
                 </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[4rem] shadow-2xl border-4 border-white overflow-hidden">
               <div className="bg-slate-900 p-12 text-white flex justify-between items-center">
                  <div className="flex items-center gap-8">
                    ...
                    <div>
                      <h4 className="text-3xl font-black uppercase tracking-tighter">Painel de Auditoria de Regras</h4>
                      <p className="text-[11px] opacity-40 font-black uppercase tracking-[0.4em]">Algoritmo de Remuneração v5.0 Master</p>
                    </div>
                  </div>
                  {!analytics.isMarginOk && <div className="bg-rose-500/20 border-2 border-rose-500 text-rose-500 px-10 py-4 rounded-[2rem] font-black text-sm animate-pulse flex items-center gap-4 uppercase"><Lock size={20}/> Margem Inválida: Pagamento Travado</div>}
               </div>
               ...
               <table className="w-full text-left">
                  ...
                  <tbody className="divide-y font-bold text-sm uppercase text-slate-700">
                     <IndicatorRow label="Escada Performance" rule={`Base: ${analytics.revPerf >= 90 ? 'ATIVADA' : 'ZERA (Abaixo de 90%)'}`} meta="90/100/110%" real={analytics.revPerf.toFixed(1)+"%"} ok={analytics.revPerf >= 90} />
                     <IndicatorRow label="Taxa de Conversão" rule="Acelerador: ≥ Meta → +0,5% na Taxa" meta={goals.conversion+"%"} real={analytics.rates.total.toFixed(1)+"%"} ok={analytics.rates.total >= goals.conversion} />
                     <IndicatorRow label="Cross Sell" rule="Acelerador: ≥ 40% → +0,5% na Taxa" meta={goals.crossSell+"%"} real={analytics.kpis.cross.toFixed(1)+"%"} ok={analytics.kpis.cross >= goals.crossSell} />
                     <IndicatorRow label="Up Sell" rule="Acelerador: ≥ 15% → +0,5% na Taxa" meta={goals.upSell+"%"} real={analytics.kpis.up.toFixed(1)+"%"} ok={analytics.kpis.up >= goals.upSell} />
                     <IndicatorRow label="Volume Atendimento" rule="Bônus Fixo: Requisito do pacote R$ 300" meta={goals.contacts} real={analytics.funnel.contato} ok={analytics.funnel.contato >= goals.contacts} />
                     <IndicatorRow label="Qualidade Pós-Venda" rule="Bônus Fixo: Requisito do pacote R$ 300" meta={goals.postSale+"%"} real={analytics.kpis.post.toFixed(1)+"%"} ok={analytics.kpis.post >= goals.postSale} />
                     <IndicatorRow label="Recuperação Clientes" rule="Bônus Fixo: Requisito do pacote R$ 300" meta={goals.reactivated} real={analytics.kpis.react} ok={analytics.kpis.react >= goals.reactivated} />
                  </tbody>
               </table>
               ...
               <div className="p-16 bg-slate-50 border-t flex flex-wrap gap-12 items-center justify-between">
                  <div className="flex flex-wrap gap-8">
                     <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Margem de Lucro %</label>
                  ...
                  <div className="flex gap-4">
                    {[1, 2, 3, 4].map(w => (
                       <div key={w} className="space-y-2">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-center block">Semana {w}</label>
        ...
        {activeTab === 'funnel' && (
          <div className="bg-white p-24 rounded-[5rem] shadow-2xl border animate-in zoom-in duration-500">
             ...
             <div className="max-w-6xl mx-auto space-y-12">
                <FunnelStep label="Lead Captado" count={analytics.funnel.contato} percent={100} color="bg-slate-300" />
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-2xl flex items-center justify-center p-6 z-[1000] animate-in fade-in duration-300">
          <div className="bg-white rounded-[6rem] p-24 max-w-4xl w-full shadow-2xl border-t-[24px] border-blue-600 animate-in zoom-in-95 relative">
            ...
            <h2 className="text-6xl font-black uppercase italic tracking-tighter text-slate-900 mb-20">Nova Oportunidade</h2>
            <div className="space-y-12">
              <input className="w-full p-10 rounded-[4rem] bg-slate-50 border-4 border-transparent focus:border-blue-500 font-black outline-none text-2xl shadow-inner" value={newLead.name} onChange={e=>setNewLead({...newLead, name: e.target.value})} placeholder="Empresa / Lead Principal" />
              <div className="grid grid-cols-2 gap-12">
                <input type="number" className="w-full p-10 rounded-[4rem] bg-slate-50 border-4 border-transparent focus:border-blue-500 font-black outline-none text-2xl shadow-inner" value={newLead.value} onChange={e=>setNewLead({...newLead, value: e.target.value})} placeholder="Valor R$" />
                <button onClick={()=>setNewLead({...newLead, reactivated: !newLead.reactivated})} className={`p-10 rounded-[4rem] font-black uppercase text-sm border-4 transition-all ${newLead.reactivated ? 'bg-emerald-500 border-emerald-500 text-white shadow-2xl' : 'bg-slate-50 border-transparent text-slate-400'}`}>É Reativação?</button>
              </div>
              <button 
                onClick={async ()=>{
                ...
                className="w-full bg-blue-600 text-white p-12 rounded-[5rem] font-black uppercase tracking-[0.5em] text-2xl shadow-2xl hover:scale-105 transition-all flex items-center justify-center gap-8 mt-10"
              >
                ATIVAR ESTRATÉGIA <ArrowRight size={40}/>
              </button>
            </div>
          </div>
        </div>
...
const QuickAction = ({ label, active, color, onClick }) => (
  <button onClick={onClick} className={`p-5 rounded-[2rem] border-2 text-[9px] font-black uppercase transition-all flex items-center justify-center ${active ? `${color} text-white border-transparent shadow-lg scale-105` : 'bg-white text-slate-300 border-slate-100'}`}>{label}</button>
);
...
const IndicatorRow = ({ label, rule, meta, real, ok }) => (
  <tr className={`group transition-all ${!ok ? 'bg-rose-50/20' : 'hover:bg-slate-50'}`}>
    ...
    <td className="p-12 text-center">
       <div className={`inline-flex items-center gap-4 px-10 py-3 rounded-full font-black text-[10px] ${ok ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}>
...
const KPIEntry = ({ title, meta, total, leads, field, format, isPercent }) => {
  ...
  return (
    <tr className="hover:bg-slate-50/50">
      ...
      <td className="p-12 text-center text-slate-300 italic">Meta: {meta}</td>
      {[1, 2, 3, 4].map(w => (
        <td key={w} className="p-12 text-center">
          <div className="flex flex-col items-center gap-3">
            <div className={`w-6 h-6 rounded-full ${getFarol(getWeeklyVal(w))} shadow-xl ring-4 ring-white`} />
            <span className="text-[11px] text-slate-400 font-black">{format(getWeeklyVal(w))}</span>
          </div>
        </td>
      ))}
      <td className="p-12 text-center bg-blue-50/40 border-l border-white shadow-inner"><span className="text-2xl font-black text-blue-900">{total}</span></td>
    </tr>
  );
};
  ...
  <div className="flex items-center gap-10">
    <div className={`h-28 ${color} rounded-[4rem] flex items-center justify-between px-16 text-white shadow-2xl transition-all hover:translate-x-4 duration-500`} style={{ width: `${Math.max(percent, 35)}%` }}>
      ...
      <div className="flex flex-col items-end">
        ...
        <span className="text-[10px] opacity-60 font-black uppercase mt-1">leads</span>
      </div>
    </div>
...
const FunnelRate = ({ value, label }) => (
  <div className="flex flex-col items-center py-6 border-l-8 border-dotted border-slate-100 ml-28">
    <div className="bg-white border-4 border-slate-50 px-10 py-4 rounded-[2rem] shadow-xl flex items-center gap-4">
       <TrendingUp className="text-blue-500" size={20}/><span className="text-slate-800 font-black text-xs uppercase tracking-[0.2em]">{label}: <span className="text-blue-600 ml-2 text-lg">{value.toFixed(1)}%</span></span>
    </div>
  </div>
);
